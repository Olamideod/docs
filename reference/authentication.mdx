...
The Authentication API handles [OAuth 2.0 ](/support/glossary#oauth-2-0) token operations. After your application redirects users to Protekt for login, you will receive an authorization code. Now we will use this API to exchange that code for access tokens, refresh expired tokens when needed, and revoke tokens when users log out.
...
Follow this security requirements for secure integration of protekt API.
